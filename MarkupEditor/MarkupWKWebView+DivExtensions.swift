...
public extension MarkupWKWebView {
    ...
    func load(divStructure: MarkupDivStructure, index _: Int = 0, handler: (() -> Void)? = nil) {
    ...
    func unload(divStructure: MarkupDivStructure, index _: Int = 0, handler: (() -> Void)? = nil) {
        for div in divStructure.divs {
    ...
    func addDiv(_ div: HtmlDivHolder, handler: (() -> Void)? = nil) {
        let id = div.id
        ...
        evaluateJavaScript("MU.addDiv('\(id)', '\(parentId)', '\(cssClass)', '\(jsonAttributes ?? "null")', '\(htmlContents)')") { _, error in
            if let error {
    ...
    func removeDiv(_ div: HtmlDivHolder, handler: (() -> Void)? = nil) {
        evaluateJavaScript("MU.removeDiv('\(div.id)')") { _, error in
            if let error {
    ...
    func addButtonGroup(_ buttonGroup: HtmlButtonGroup, handler: (() -> Void)? = nil) {
    ...
    func removeButtonGroup(_ buttonGroup: HtmlButtonGroup, handler: (() -> Void)? = nil) {
        evaluateJavaScript("MU.removeDiv('\(buttonGroup.id)')") { _, error in
            if let error {
    ...
    private func addButton(_ button: HtmlButton, in parentId: String, handler: (() -> Void)? = nil) {
        ...
        evaluateJavaScript("MU.addButton('\(id)', '\(parentId)', '\(cssClass)', '\(label)')") { _, error in
            if let error {
    ...
    private func removeButton(_ button: HtmlButton, handler: (() -> Void)? = nil) {
        evaluateJavaScript("MU.removeButton('\(button.id)')") { _, error in
            if let error {
    ...
    func focus(on id: String?, handler: (() -> Void)? = nil) {
        guard let id else {
            handler?()
            return
        }
        DispatchQueue.main.async {
            self.evaluateJavaScript("MU.focusOn('\(id)')") { _, error in
                if let error {
    ...
    func scrollIntoView(id: String?, handler: (() -> Void)? = nil) {
        guard let id else {
            handler?()
            return
        }
        evaluateJavaScript("MU.scrollIntoView('\(id)')") { _, error in
            if let error {
    ...
    func removeAllDivs(handler: (() -> Void)? = nil) {
        evaluateJavaScript("MU.removeAllDivs()") { _, error in
            if let error {
